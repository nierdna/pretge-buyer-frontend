...
export default function toPercentString(
  ...
  options?: ToPercentStringOptions
): string {
  try {
    if (n == null) return '--';
    const stringPart = (Number(n) * ((options?.alreadyPercented ?? true) ? 1 : 100)).toFixed(
      options?.decimals ?? 2
    );
    if ((n === 0 || n === '0') && !options?.notShowZero) return '0%';
    if (!options?.exact && stringPart === (0).toFixed(options?.decimals ?? 2))
    ...
    return options?.alwaysSigned
      ...
      : `${formatNumber(stringPart, {
          maxDecimalCount: options?.decimals,
          decimalMode: options?.decimalMode || 'trim',
        })}%`;
  } catch (err) {
    return '0%';
  }
