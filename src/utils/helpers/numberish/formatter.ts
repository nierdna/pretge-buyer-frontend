...
export const numTransform = (value: unknown) => (isNaN(value as number) ? 0 : value);
...
function decimalTrailingZeroesToExponent(
  formattedCurrency: string,
  maximumDecimalTrailingZeroes: number
): string {
  const decimalTrailingZeroesPattern = new RegExp(
    `(\\.|,)(0{${maximumDecimalTrailingZeroes + 1},})(?=[1-9]?)`
  );
  return formattedCurrency.replace(
    decimalTrailingZeroesPattern,
    (_match, separator, decimalTrailingZeroes) =>
      `${separator}0${toSubscript(decimalTrailingZeroes.length)}`
  );
}

function formatCurrencyOverride(
  formattedCurrency: string,
  maximumDecimalTrailingZeroes?: number
): string {
  if (typeof maximumDecimalTrailingZeroes !== 'undefined') {
    formattedCurrency = decimalTrailingZeroesToExponent(
      formattedCurrency,
...
function generateIntlNumberFormatter(
  ...
  return {
    format: (value: number) => {
      ...
      if (abbreviated) {
        const matches = formatWithAbbreviation(value, numDecimals).match(
          /^(\d+(?:\.\d+)?)([TBMK]?)$/
        );
        if (matches) {
...
export function formatCurrency(
  ...
  const amountDecimal =
    amount instanceof Decimal ? amount : new Decimal(String(amount).replace(/,/gi, ''));
  ...
  const currencyFormatterNoDecimal: { format: (value: number) => string } =
    generateIntlNumberFormatter(symbol, abbreviated, 0);
  ...
  if (Object.prototype.hasOwnProperty.call(params, 'decimalPlaces')) {
    ...
    return formatCurrencyOverride(
      currencyFormatterCustom.format(amountNumber),
      maximumDecimalTrailingZeroes
    );
  }
  ...
  if (amountNumber === 0.0) {
    ...
    const currencyFormatterMedium: { format: (value: number) => string } =
      generateIntlNumberFormatter(symbol, abbreviated, 3);
    return formatCurrencyOverride(
    ...
    const currencyFormatterSmall: { format: (value: number) => string } =
      generateIntlNumberFormatter(symbol, abbreviated, 6);
    return formatCurrencyOverride(
    ...
    } = generateIntlNumberFormatter(symbol, abbreviated, 12);
    return formatCurrencyOverride(
    ...
    const currencyFormatterTooSmall: { format: (value: number) => string } =
      generateIntlNumberFormatter(symbol, abbreviated, digitsAfterPoint);
    return formatCurrencyOverride(
