'use client';

import { ITokenProjectExternal } from '@/types/tokenProject';
import { Calendar, Tag } from 'lucide-react';

interface TokenTGEProps {
  tokenExternal?: ITokenProjectExternal | null;
}

export default function TokenTGE({ tokenExternal }: TokenTGEProps) {
  const tgeInfo = tokenExternal?.data?.tgeInfo;

  // Format date
  const formatDate = (dateString?: string) => {
    if (!dateString) return '--';
    try {
      const date = new Date(dateString);
      return date.toLocaleDateString('en-US', {
        month: 'numeric',
        day: 'numeric',
        year: 'numeric',
      });
    } catch {
      return '--';
    }
  };

  const tgeData = [
    {
      icon: <Calendar className="h-6 w-6" />,
      label: 'TGE Date',
      value: formatDate(tgeInfo?.tgeDate) || '9/25/2025',
    },
    {
      icon: <Tag className="h-6 w-6" />,
      label: 'TGE Exchange',
      value: tgeInfo?.tgeExchange || '--',
    },
    {
      icon: (
        <svg className="h-6 w-6" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10s10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19c-.14.75-.42 1-.68 1.03c-.58.05-1.02-.38-1.58-.75c-.88-.58-1.38-.94-2.23-1.5c-.99-.65-.35-1.01.22-1.59c.15-.15 2.71-2.48 2.76-2.69a.2.2 0 0 0-.05-.18c-.06-.05-.14-.03-.21-.02c-.09.02-1.49.95-4.22 2.79c-.4.27-.76.41-1.08.4c-.36-.01-1.04-.2-1.55-.37c-.63-.2-1.12-.31-1.08-.66c.02-.18.27-.36.74-.55c2.92-1.27 4.86-2.11 5.83-2.51c2.78-1.16 3.35-1.36 3.73-1.36c.08 0 .27.02.39.12c.1.08.13.19.14.27c-.01.06.01.24 0 .38z" />
        </svg>
      ),
      label: 'TGE Date',
      value: formatDate(tgeInfo?.tgeDate) || '9/25/2025',
    },
  ];

  return (
    <div className="p-4 md:p-6">
      <div className="mb-4 flex items-center gap-4 md:mb-6">
        <Calendar className="h-5 w-5 text-primary-text md:h-6 md:w-6" />
        <h2 className="text-base font-semibold text-primary-text md:text-lg">TGE Information</h2>
      </div>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-6">
        {tgeData.map((item, index) => (
          <div key={index} className="flex items-center gap-4">
            <div className="flex h-[42px] w-[42px] items-center justify-center rounded-md border border-border bg-card text-primary-text">
              {item.icon}
            </div>
            <div className="min-w-[101px]">
              <div className="text-sm text-secondary-text">{item.label}</div>
              <div className="text-xl font-semibold text-primary-text">{item.value}</div>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
