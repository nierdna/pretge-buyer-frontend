...
export default function TokenSocial({ tokenExternal }: TokenSocialProps) {
  const communityMetrics = tokenExternal?.data?.communityMetrics;
  ...
  const socialStats = [
    {
      ...
      value: communityMetrics?.twitterFollowers?.toLocaleString() || '46,072',
    },
    {
      icon: (
        <svg className="h-6 w-6" viewBox="0 0 24 24" fill="currentColor">
      ...
      value: communityMetrics?.discordMembers?.toLocaleString() || '46,072',
    },
    {
      icon: (
        <svg className="h-6 w-6" viewBox="0 0 24 24" fill="currentColor">
      ...
      value: communityMetrics?.telegramMembers?.toLocaleString() || '46,072',
    },
    {
      icon: (
        <svg className="h-6 w-6" viewBox="0 0 24 24" fill="currentColor">
      ...
      label: 'GitHub Stars',
      value: communityMetrics?.githubStars?.toLocaleString() || '0',
    },
  ];
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-6">
        {socialStats.map((stat, index) => (
          <div key={index} className="flex items-center gap-4">
            <div className="flex h-[42px] w-[42px] items-center justify-center rounded-md border border-border bg-card">
              <div className="text-primary-text">{stat.icon}</div>
            </div>
            <div className="min-w-[101px]">
              <div className="text-xl font-semibold text-primary-text">{stat.value}</div>
